...
const CAMERA_HORIZONTAL_ROTATION_SPEED = 2 * PI;
const CAMERA_VERTICAL_ROTATION_SPEED = PI;
...
fn update_rotation_horizontal() {
    camera.horizontal_angle -= std_.mouse.delta.x / f32(std_.surface.size.x) * CAMERA_HORIZONTAL_ROTATION_SPEED;
...
fn update_rotation_vertical() {
    ...
    camera.vertical_angle -= std_.mouse.delta.y / f32(std_.surface.size.y) * CAMERA_VERTICAL_ROTATION_SPEED;
...
fn update_position() {
    ...
    camera.position += normalize_vec3f_or_zero(vec3f(rotated_delta.x, 0, rotated_delta.z)) * CAMERA_MOVEMENT_SPEED;
}
