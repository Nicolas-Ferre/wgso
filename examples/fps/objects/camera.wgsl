...
fn init_camera() -> Camera {
    ...
    let far = 100.;
    let near = 0.01;
    return Camera(position, horizontal_angle, vertical_angle, rotation, fov, far, near);
}
...
fn _update_camera_horizontal_rotation(camera: Camera) -> Camera {
    ...
    updated.rotation = quat(vec3f(0, 1, 0), updated.horizontal_angle);
    return updated;
}

fn _update_camera_vertical_rotation(camera: Camera) -> Camera {
    var updated = camera;
    let direction = (rotation_mat(updated.rotation) * vec4f(1, 0, 0, 1)).xyz;
